...
class Config:
    ...
    num_train_epochs = 5
    # train batch size and valid batch size need to be equal in pipeline parallelism
    per_device_train_batch_size = 2
    per_device_valid_batch_size = 2
    microbatch_size = 1 # For pipeline parallelism
...
# Training function
# -------------------------------------------------
def train_epoch(rank, model, schedule_pipe, dataloader, optimizer, scheduler, scaler, device, epoch, config):
...
# -------------------------------------------------
# Distributed environment setup functions
# -------------------------------------------------
def setup_dist_env(rank, world_size):
...
def distributed_training(rank, world_size):
    ...
    setup_dist_env(rank, world_size)
    ...
    try:
        ...
        for epoch in range(config.num_train_epochs):
            ...
            train_loss = train_epoch(rank, model, schedule_pipe, train_dataloader, optimizer, scheduler, scaler, device, epoch, config)
